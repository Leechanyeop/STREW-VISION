# JETSON_ROBOT 수정 구축 계획 (v5, 2026-07-15 5차 개정 반영)

이전 버전(v3)은 아키텍처 전환의 1차 개정(결정권 이전, `ASSIGN_TARGET` 단일 셀 지정)까지만 반영되어 있었다. 그 사이 실시간으로 2~5차 개정이 이어져 순회 사이클 관리 전체, ERROR 복구 경로 도입과 그 이후의 단순화(severity/RESET 제거)까지 확정됐고, 그 김에 남아있던 정리 항목(설정값 불일치, 죽은 코드)도 함께 마무리했다. 이 버전은 그 전체를 반영한다.
...
| **ERROR 복구 경로(4차 개정 → 5차 개정에서 단순화)** | **완료.** 처음엔 `ERROR`에 `severity`(minor/critical) 필드를 붙이고 minor일 때만 Jetson이 `RESET`으로 원격 재시작하는 경로를 만들었으나, 물리 센서(전류/엔코더 등)가 없어 Mega가 그 구분을 스스로 할 근거가 없다는 게 확인되어 severity/`RESET`을 통째로 제거. 지금은 ERROR = 항상 물리 리셋으로만 복구. |
...
| **Mega 개발자용 PDF 안내** | **v4까지 완료, v5 필요.** v4는 severity/RESET을 포함한 8종 메시지 기준이라 5차 개정(severity/RESET 제거, 7종)으로 이제 stale — v5로 갱신 예정. |
...
(오류 시) Mega -> Jetson: ERROR(reason)
-> 항상 물리 리셋(전원 재시작 등)으로만 복구 (severity/RESET은 5차 개정에서 제거됨)
...
| **Mega 펌웨어 신규 프로토콜 반영** | **보류(최우선순위 아님)** | `mega_firmware.ino`가 아직 구버전 그대로. `ACTION_MAP` 로직 C++ 포팅 + 7종 메시지 송수신 + IDLE/RUN/ERROR 상태머신 필요. severity 판정/RESET 거부 로직은 5차 개정에서 필요 없어짐(항상 물리 리셋). 외부 개발자에게 안내 예정(PDF v5). |
...
| ERROR 복구 | `robot/state_machine.py` | **단순화 완료** — severity/RESET 제거, 항상 물리 리셋(전원 재시작 등)으로만 복구 |
| 메시지 타입 상수 | `robot/command.py`의 `MSG_*` (7종) | **구현 완료**, 구버전 `ArduinoCommand`는 삭제됨 |
...
3. 시간 되면 `mega_firmware.ino`에 신규 프로토콜(7종 메시지 + IDLE/RUN/ERROR, ERROR는 항상 물리 리셋) 포팅.
